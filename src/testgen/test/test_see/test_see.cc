...
public:
  ...
  void execute() {
    ...
    SEE see(std::move(functionFactory));
...
protected:
  Program makeProgram() override {
    ...
    statements.push_back(
        TestUtils::makeAssumeEq(make_unique<Var>("x"), make_unique<Num>(5)));
  ...
  void verify(SEE &see, map<string, int> &model, bool isSat) override {
    ...
    assert(sigma.getValue("z")->exprType == ExprType::FUNC_CALL_EXPR);
...
protected:
  Program makeProgram() override {
    ...
    statements.push_back(make_unique<Assign>(
        make_unique<Var>("U"), make_unique<Set>(std::move(elements))));
    ...
    // assume(u > 0)
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Gt", make_unique<Var>("u"), make_unique<Num>(0))));

    // assume(u < 10)
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Lt", make_unique<Var>("u"), make_unique<Num>(10))));
...
protected:
  Program makeProgram() override {
    ...
    statements.push_back(make_unique<Assign>(
        make_unique<Var>("S"), make_unique<Set>(std::move(elements))));
    ...
    statements.push_back(
        make_unique<Assume>(make_unique<FuncCall>("in", std::move(inArgs))));
...
protected:
  Program makeProgram() override {
    ...
    statements.push_back(make_unique<Assign>(
        make_unique<Var>("S1"), make_unique<Set>(std::move(elements1))));
    ...
    statements.push_back(make_unique<Assign>(
        make_unique<Var>("S2"), make_unique<Set>(std::move(elements2))));
    ...
    // assume(x > 2)
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Gt", make_unique<Var>("x"), make_unique<Num>(2))));
...
protected:
  Program makeProgram() override {
    ...
    statements.push_back(make_unique<Assign>(
        make_unique<Var>("t"), make_unique<Tuple>(std::move(tupleExprs))));
    ...
    // assume(x < 15)
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Lt", make_unique<Var>("x"), make_unique<Num>(15))));
...
protected:
  Program makeProgram() override {
    ...
    // assume(x > 10)
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Gt", make_unique<Var>("x"), make_unique<Num>(10))));
...
int main() {
  vector<SEETest *> testcases = {new SEETest1(), new SEETest2(), new SEETest3(),
                                 new SEETest4(), new SEETest5(), new SEETest6(),
                                 new SEETest7(), new SEETest8(), new SEETest9(),
                                 new SEETest10()};
  ...
  cout << "\n========================================" << endl;
  cout << "SEE Test Results: " << passed << " passed, " << failed << " failed"
       << endl;
  cout << "========================================" << endl;
