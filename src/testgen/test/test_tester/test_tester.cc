...
public:
  ...
  void printProgram(const string &title, const Program &prog) {
    ...
    for (size_t i = 0; i < prog.statements.size(); i++) {
      ...
      if (prog.statements[i]->statementType == StmtType::ASSIGN) {
        ...
        cout << (leftVar ? leftVar->name : "?")
             << " := " << TestUtils::exprToString(assign.right.get()) << endl;
      } else if (prog.statements[i]->statementType == StmtType::ASSUME) {
        Assume &assume = dynamic_cast<Assume &>(*prog.statements[i]);
        cout << "assume(" << TestUtils::exprToString(assume.expr.get()) << ")"
             << endl;
      }
    }
  }
  ...
  void execute() {
    ...
    unique_ptr<Program> atcCopy = make_unique<Program>(std::move(
        const_cast<vector<unique_ptr<Stmt>> &>(abstractProgram.statements)));
    unique_ptr<Program> concreteProgram =
    ...
    unique_ptr<Program> atcCopy2 = make_unique<Program>(std::move(
        const_cast<vector<unique_ptr<Stmt>> &>(abstractProgram2.statements)));

    printProgram(
        "\n[2] Final Concrete Test Case (CTC - after full symbolic execution):",
        *concreteProgram);
...
protected:
  Program makeAbstractProgram() override {
    ...
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Gt", make_unique<Var>("x"), make_unique<Num>(5))));
  ...
  void verify(Tester &tester, unique_ptr<Program> &result) override {
    ...
    // Verify path constraints were generated (may be more than 1 due to
    // recursive calls)
    ...
    cout
        << "Verification: Abstract test case successfully converted to concrete"
        << endl;
  }
};
...
public:
  TesterTest2()
      : TesterTest("Abstract test case with two inputs and constraints") {}
...
protected:
  Program makeAbstractProgram() override {
    ...
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Gt", make_unique<Var>("x"), make_unique<Num>(3))));
  ...
  void verify(Tester &tester, unique_ptr<Program> &result) override {
    ...
    // Verify path constraints were generated (may be more than 2 due to
    // recursive calls)
    vector<Expr *> &pathConstraints = tester.getPathConstraints();
    ...
    cout << "Verification: Two-input abstract test case successfully converted"
         << endl;
  }
};
...
protected:
  Program makeAbstractProgram() override {
    ...
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("x"), make_unique<Num>(5)));

    statements.push_back(
        make_unique<Assign>(make_unique<Var>("y"), make_unique<Num>(10)));
...
protected:
  Program makeAbstractProgram() override {
    ...
    statements.push_back(
        TestUtils::makeAssumeEq(make_unique<Var>("z"), make_unique<Num>(10)));
  ...
  void verify(Tester &tester, unique_ptr<Program> &result) override {
    ...
    cout << "Verification: Abstract test case with computation successfully "
            "converted"
         << endl;
  }
};
...
public:
  ...
  virtual void execute() {
    ...
    unique_ptr<Program> atc = make_unique<Program>(std::move(
        const_cast<vector<unique_ptr<Stmt>> &>(abstractProgram.statements)));
...
protected:
  ...
  vector<unique_ptr<Expr>> makeConcreteVals() override { return {}; }
...
protected:
  Program makeAbstractProgram() override {
    ...
    statements.push_back(TestUtils::makeInputAssign("x"));
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("y"), make_unique<Num>(5)));
    statements.push_back(TestUtils::makeInputAssign("z"));
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("w"),
                            TestUtils::makeBinOp("Add", make_unique<Var>("x"),
                                                 make_unique<Var>("y"))));
    return Program(std::move(statements));
  }
  ...
  void verify(unique_ptr<Program> &result) override {
    ...
    assert(assign4.right->exprType == ExprType::FUNC_CALL_EXPR);
  }
};
...
protected:
  ...
  void verify(unique_ptr<Program> &result) override {
    ...
    Var *leftVar3 = dynamic_cast<Var *>(assign3.left.get());
    assert(leftVar3 && leftVar3->name == "z");
    assert(assign3.right->exprType == ExprType::FUNC_CALL_EXPR);
...
public:
  RewriteATCTest6()
      : RewriteATCTest("rewriteATC with empty program and empty values") {}
...
public:
  RewriteATCTest7()
      : RewriteATCTest("rewriteATC error case (empty program with values)") {}
...
protected:
  Program makeAbstractProgram() override {
    ...
    statements.push_back(TestUtils::makeInputAssign("x"));
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Gt", make_unique<Var>("x"), make_unique<Num>(0))));
    statements.push_back(TestUtils::makeInputAssign("y"));
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Lt", make_unique<Var>("y"), make_unique<Num>(100))));
    return Program(std::move(statements));
  }
...
public:
  RewriteATCTest9()
      : RewriteATCTest("rewriteATC preserves non-input function calls") {}
...
protected:
  Program makeAbstractProgram() override {
    ...
    statements.push_back(TestUtils::makeInputAssign("x"));
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("y"),
                            TestUtils::makeBinOp("Add", make_unique<Var>("x"),
                                                 make_unique<Num>(5))));
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("z"),
                            TestUtils::makeBinOp("Mul", make_unique<Var>("y"),
                                                 make_unique<Num>(2))));
    return Program(std::move(statements));
  }
  ...
  void verify(unique_ptr<Program> &result) override {
    ...
    Var *leftVar2 = dynamic_cast<Var *>(assign2.left.get());
    assert(leftVar2 && leftVar2->name == "y");
    assert(assign2.right->exprType == ExprType::FUNC_CALL_EXPR);
...
public:
  TesterTest5()
      : TesterTest("Complex test with API function calls and interruption") {}
...
protected:
  Program makeAbstractProgram() override {
    ...
    // y := 0
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("y"), make_unique<Num>(0)));

    // F1 sequence
    // y1 := y
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("y1"), make_unique<Var>("y")));
    ...
    // assume(x1 < 10)
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Lt", make_unique<Var>("x1"), make_unique<Num>(10))));
    ...
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("r1"),
    ...
    //     TestUtils::makeBinOp("Eq", make_unique<Var>("r1"),
    //     make_unique<Num>(1))
    // ));
    ...
    //         make_unique<Var>("y"),
    //         TestUtils::makeBinOp("Add", make_unique<Var>("y1"),
    //         make_unique<Var>("x1"))
    //     )
    // ));
    ...
    // y2 := y
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("y2"), make_unique<Var>("y")));
    ...
    // assume(x2 < 10)
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Lt", make_unique<Var>("x2"), make_unique<Num>(10))));
    ...
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("r2"),
                            make_unique<FuncCall>("f1", std::move(f2Args))));

    // // assume(r2 = 1)
    // statements.push_back(make_unique<Assume>(
    //     TestUtils::makeBinOp("Eq", make_unique<Var>("r2"),
    //     make_unique<Num>(1))
    // ));
    ...
    //     TestUtils::makeBinOp("Eq",
    //         make_unique<Var>("y"),
    //         TestUtils::makeBinOp("Add", make_unique<Var>("y2"),
    //         make_unique<Var>("x2"))
    //     )
    // ));
...
protected:
  Program makeAbstractProgram() override {
    ...
    // assume(y > 0)
    statements.push_back(make_unique<Assume>(TestUtils::makeBinOp(
        "Gt", make_unique<Var>("y"), make_unique<Num>(0))));
    ...
    statements.push_back(
        make_unique<Assign>(make_unique<Var>("z"),
                            TestUtils::makeBinOp("Add", make_unique<Var>("r1"),
                                                 make_unique<Var>("r2"))));
...
int main() {
  ...
  vector<TesterTest *> integrationTests = {// new TesterTest1(),
                                           // new TesterTest2(),
                                           // new TesterTest3(),
                                           // new TesterTest4(),
                                           new TesterTest5(), new TesterTest6(),
                                           new TesterTest7()};
