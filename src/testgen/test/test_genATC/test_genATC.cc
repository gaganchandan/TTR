 ...
 * expr Format: (_RESPONSE_CODE_XXX, expr) or just _RESPONSE_CODE_XXX
 */
unique_ptr<Expr> makeResponseExpr(HTTPResponseCode code,
...
protected:
  ...
  TypeMap makeTypeMap() override {
    TypeMap typeMap;
    // U: map<string, string>
    typeMap.setValue("U", new MapType(make_unique<TypeConst>("string"),
                                      make_unique<TypeConst>("string")));
    return typeMap;
  }
...
protected:
  unique_ptr<Spec> makeSpec() override {
    ...
    auto apiCall = make_unique<APIcall>(
        std::move(call), Response(HTTPResponseCode::OK_200, nullptr));

    blocks.push_back(make_unique<API>("signup", std::move(pre),
                                      std::move(apiCall), nullptr));
  ...
  vector<string> makeTestString() override { return {"signup"}; }
  ...
  void verify(const Program &atc) override {
    ...
    // 5. _result0 = signup(u0, p0) (API call) - LHS is Var with tuple type in
    // TypeMap
    ...
    const Assign *input1 =
        dynamic_cast<const Assign *>(atc.statements[1].get());
    ...
    assert(input1->right->exprType == ExprType::FUNC_CALL_EXPR);

    const Assign *input2 =
        dynamic_cast<const Assign *>(atc.statements[2].get());
    ...
    const Assume *assume =
        dynamic_cast<const Assume *>(atc.statements[3].get());
    assert(assume != nullptr);
    assert(assume->expr->exprType == ExprType::FUNC_CALL_EXPR);
    const FuncCall *assumeCall =
        dynamic_cast<const FuncCall *>(assume->expr.get());
    ...
    const Assign *apiCall =
        dynamic_cast<const Assign *>(atc.statements[4].get());
    ...
    assert(apiCall->right->exprType == ExprType::FUNC_CALL_EXPR);
    const FuncCall *callFunc =
        dynamic_cast<const FuncCall *>(apiCall->right.get());
...
public:
  GenATCTest3()
      : GenATCTest("API block with primed variables - state transition") {}
...
protected:
  unique_ptr<Spec> makeSpec() override {
    ...
    mapEntries.push_back(
        make_pair(make_unique<Var>("u"), make_unique<Var>("p")));
    ...
    blocks.push_back(make_unique<API>("signup", std::move(pre),
                                      std::move(apiCall), std::move(post)));
  ...
  void verify(const Program &atc) override {
    ...
    for (size_t i = 3; i < atc.statements.size(); i++) {
      const Assign *assign =
          dynamic_cast<const Assign *>(atc.statements[i].get());
    ...
    for (size_t i = 0; i < atc.statements.size(); i++) {
      const Assert *assertStmt =
          dynamic_cast<const Assert *>(atc.statements[i].get());
      if (assertStmt) {
        foundAssert = true;
...
protected:
  unique_ptr<Spec> makeSpec() override {
    ...
    // Block 2: login(u, p)
    {
      ...
      blocks.push_back(make_unique<API>("login", std::move(pre),
                                        std::move(apiCall), nullptr));
    }
  ...
  TypeMap makeTypeMap() override {
    ...
    typeMap.setValue("U", new MapType(make_unique<TypeConst>("string"),
                                      make_unique<TypeConst>("string")));
    typeMap.setValue("T", new MapType(make_unique<TypeConst>("string"),
                                      make_unique<TypeConst>("string")));
    // Local variables: u, p: string
    typeMap.setValue("u", new TypeConst("string"));
    typeMap.setValue("p", new TypeConst("string"));
  ...
  vector<string> makeTestString() override { return {"signup", "login"}; }
 ...
 */
int main() {
  vector<GenATCTest *> testcases = {new GenATCTest1(), new GenATCTest2(),
                                    new GenATCTest3(), new GenATCTest4()};
