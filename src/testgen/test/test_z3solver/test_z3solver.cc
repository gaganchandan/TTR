    ...
    if (result.isSat) {
      ...
      for (const auto &entry : result.model) {
        if (entry.second->type == ResultType::INT) {
          const IntResultValue *intVal =
              dynamic_cast<const IntResultValue *>(entry.second.get());
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    unique_ptr<Expr> addExpr =
        TestUtils::makeBinOp("Add", x0.get()->clone(), x1.get()->clone());
    unique_ptr<Expr> eqConstraint =
        TestUtils::makeBinOp("Eq", std::move(addExpr), make_unique<Num>(10));

    // Create constraint: X0 > 3
    unique_ptr<Expr> gtConstraint =
        TestUtils::makeBinOp("Gt", x0.get()->clone(), make_unique<Num>(3));

    // Conjoin: (X0 + X1 = 10) AND (X0 > 3)
    return TestUtils::makeBinOp("And", std::move(eqConstraint),
                                std::move(gtConstraint));
  }
  ...
  void verify(const Result &result) override {
    ...
    cout << "Verification: Solution satisfies (X0 + X1 = 10) AND (X0 > 3)"
         << endl;
  }
};
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    unique_ptr<Expr> eq5 =
        TestUtils::makeBinOp("Eq", x0.get()->clone(), make_unique<Num>(5));

    // Create constraint: X0 = 10
    unique_ptr<Expr> eq10 =
        TestUtils::makeBinOp("Eq", x0.get()->clone(), make_unique<Num>(10));
  ...
  void verify(const Result &result) override {
    ...
    cout << "Verification: Correctly identified contradictory constraints"
         << endl;
  }
};
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    unique_ptr<Expr> add1 =
        TestUtils::makeBinOp("Add", x0.get()->clone(), x1.get()->clone());
    unique_ptr<Expr> eq15 =
        TestUtils::makeBinOp("Eq", std::move(add1), make_unique<Num>(15));

    // Create constraint: X1 + X2 = 20
    unique_ptr<Expr> add2 =
        TestUtils::makeBinOp("Add", x1.get()->clone(), x2.get()->clone());
    unique_ptr<Expr> eq20 =
        TestUtils::makeBinOp("Eq", std::move(add2), make_unique<Num>(20));

    // Create constraint: X0 < X1
    unique_ptr<Expr> ltConstraint =
        TestUtils::makeBinOp("Lt", x0.get()->clone(), x1.get()->clone());

    // Conjoin all: (X0 + X1 = 15) AND (X1 + X2 = 20) AND (X0 < X1)
    unique_ptr<Expr> and1 =
        TestUtils::makeBinOp("And", std::move(eq15), std::move(eq20));
    return TestUtils::makeBinOp("And", std::move(and1),
                                std::move(ltConstraint));
  }
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    unique_ptr<Expr> mulExpr =
        TestUtils::makeBinOp("Mul", x0.get()->clone(), x1.get()->clone());
    unique_ptr<Expr> eqConstraint =
        TestUtils::makeBinOp("Eq", std::move(mulExpr), make_unique<Num>(12));

    // Create constraint: X0 > 2
    unique_ptr<Expr> gt1 =
        TestUtils::makeBinOp("Gt", x0.get()->clone(), make_unique<Num>(2));

    // Create constraint: X1 > 2
    unique_ptr<Expr> gt2 =
        TestUtils::makeBinOp("Gt", x1.get()->clone(), make_unique<Num>(2));

    // Conjoin all: (X0 * X1 = 12) AND (X0 > 2) AND (X1 > 2)
    unique_ptr<Expr> and1 =
        TestUtils::makeBinOp("And", std::move(eqConstraint), std::move(gt1));
  ...
  void verify(const Result &result) override {
    ...
    cout << "Verification: Solution satisfies (X0 * X1 = 12) AND (X0 > 2) AND "
            "(X1 > 2)"
         << endl;
  }
};
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    // Create constraint: X0 > 10
    unique_ptr<Expr> gt =
        TestUtils::makeBinOp("Gt", x0.get()->clone(), make_unique<Num>(10));

    // Create constraint: X0 < 5
    unique_ptr<Expr> lt =
        TestUtils::makeBinOp("Lt", x0.get()->clone(), make_unique<Num>(5));
  ...
  void verify(const Result &result) override {
    ...
    cout << "Verification: Correctly identified impossible range constraint"
         << endl;
  }
};
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    unique_ptr<Expr> subExpr =
        TestUtils::makeBinOp("Sub", x0.get()->clone(), x1.get()->clone());
    unique_ptr<Expr> eqConstraint =
        TestUtils::makeBinOp("Eq", std::move(subExpr), make_unique<Num>(5));
  ...
  void verify(const Result &result) override {
    ...
    cout << "Verification: Solution satisfies (X0 - X1 = 5) AND (X0 = 10)"
         << endl;
  }
};
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    auto gtZero =
        TestUtils::makeBinOp("Gt", x.get()->clone(), make_unique<Num>(0));
    auto ltTen =
        TestUtils::makeBinOp("Lt", x.get()->clone(), make_unique<Num>(10));

    auto combined =
        TestUtils::makeBinOp("And", std::move(notInExpr), std::move(gtZero));
  ...
  void verify(const Result &result) override {
    ...
    for (const auto &entry : result.model) {
      if (entry.second->type == ResultType::INT) {
        int val =
            dynamic_cast<const IntResultValue *>(entry.second.get())->value;
  ...
  void verify(const Result &result) override {
    ...
    for (const auto &entry : result.model) {
      if (entry.second->type == ResultType::INT) {
        ...
        assert(val == 3 || val == 4);
        cout << "Verification: x = " << val
             << " is in union({1,2}, {3,4}) and > 2" << endl;
      }
    }
  }
};
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    notInArgs.push_back(x.get()->clone());
    ...
    auto eq42 =
        TestUtils::makeBinOp("Eq", x.get()->clone(), make_unique<Num>(42));
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    pairs.push_back(make_pair(make_unique<Var>("key1"), make_unique<Num>(100)));
    ...
    // get(M, "key1") = 100
    return TestUtils::makeBinOp("Eq", std::move(getExpr),
                                make_unique<Num>(100));
  }
...
protected:
  unique_ptr<Expr> makeConstraint() override {
    ...
    vector<pair<unique_ptr<Var>, unique_ptr<Expr>>> pairs;
    pairs.push_back(make_pair(make_unique<Var>("10"), // Will be treated as int
                              make_unique<Num>(100)));
    auto mapExpr = make_unique<Map>(std::move(pairs));
    ...
    // get(put(M, 5, v), 5) = v
    return TestUtils::makeBinOp("Eq", std::move(getExpr), v.get()->clone());
  }
  ...
  void verify(const Result &result) override {
    ...
    for (const auto &entry : result.model) {
      if (entry.second->type == ResultType::INT) {
        ...
        assert(val == 2 || val == 3);
        cout << "Verification: x = " << val
             << " is in intersection({1,2,3}, {2,3,4})" << endl;
      }
    }
  }
};
  ...
  void verify(const Result &result) override {
    ...
    for (const auto &entry : result.model) {
      if (entry.second->type == ResultType::INT) {
        ...
        assert(val == 1 || val == 3);
        cout << "Verification: x = " << val
             << " is in difference({1,2,3,4}, {2,4})" << endl;
      }
    }
  }
};
...
int main() {
  vector<Z3Test *> testcases = {new Z3Test1(),  new Z3Test2(),  new Z3Test3(),
                                new Z3Test4(),  new Z3Test5(),  new Z3Test6(),
                                new Z3Test7(),  new Z3Test8(),  new Z3Test9(),
                                new Z3Test10(), new Z3Test11(), new Z3Test12(),
                                new Z3Test13(), new Z3Test14()};
